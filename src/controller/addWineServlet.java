 ...
 */
@WebServlet("/addWine")
@MultipartConfig(location="C:/Users/zd2H12/temp")
public class addWineServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String strAlcohol = request.getParameter("alcohol");
		...
		try {
			...
			isValidated = false;
			request.setAttribute("errPrice", "※整数で入力");
		}

		// アルコールのバリデーション
				Integer alcohol = null;
				try {
					alcohol = Integer.parseInt(strAlcohol);
					if ( alcohol < 0 || alcohol > 16) {
						isValidated = false;
						request.setAttribute("errAlcohol", "※不適切な数値");
					}
				} catch (NumberFormatException e) {
					isValidated = false;
					request.setAttribute("errAlcohol", "※整数で入力");
				}
		...
		request.setAttribute("alcohol", strAlcohol);
